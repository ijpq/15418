...
void usage(const char *progname) {
    ...
    printf("  -m  --test <TYPE>      Run specified function on input.  Valid tests are: scan, find_peaks\n");
...
int main(int argc, char **argv) {
    ...
    if (test.compare("scan") == 0) { // test exclusive scan
        ...
        int cu_size;
        for (int i = 0; i < 3; i++) {
            cudaTime = std::min(cudaTime, cudaFindPeaks(inarray, N, resultarray, &cu_size));
        }
        ...
        if (serial_size != cu_size) {
            fprintf(stderr, "Error: Device find_peaks outputs incorrect size. "
                            "Expected %d, got %d.\n",
                    serial_size, cu_size);
            exit(1);
        }
        for (int i = 0; i < serial_size; i++) {
            if (checkarray[i] != resultarray[i]) {
                fprintf(stderr, "Error: Device find_peaks outputs incorrect result."
                                " A[%d] = %d, expecting %d.\n",
                        i, resultarray[i], checkarray[i]);
                exit(1);
            }
